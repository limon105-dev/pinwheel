...
var path = {
  src: {
    ...
    js: "source/js/*.js",
    scss: ["source/scss/**/*.scss", "!source/scss/bootstrap/**/*.scss"],
    bootstrapSCSS: "source/scss/bootstrap/**/*.scss",
...
gulp.task("html", function () {
  return gulp
    ...
    .pipe(
      comments(`
      WEBSITE: https://limon105-dev.github.io/Limon-Personal-Portfolio/
      LINKEDIN: https://www.linkedin.com/in/limon-ali/
      FACEBOOK: https://www.facebook.com/limon.ali.549/
      GITHUB: https://github.com/limon105-dev
    `)
    )
...
// Bootstrap SCSS
gulp.task("bootstrapSCSS", function () {
  return gulp
    .src("source/scss/bootstrap/bootstrap.scss")
    .pipe(sourcemaps.init())
    .pipe(
      sass({
        outputStyle: "expanded",
...
// Copy SCSS files into download folder
gulp.task("scss-files", function () {
  return gulp.src(path.src.scss).pipe(gulp.dest(path.build.dir + "scss/"));
});

// Copy Bootstrap SCSS files into download folder
gulp.task("bootstrap-scss-files", function () {
  return gulp
    .src(path.src.bootstrapSCSS)
    .pipe(gulp.dest(path.build.dir + "scss/bootstrap/"));
});
...
gulp.task("watch", function () {
  ...
  gulp.watch(path.src.bootstrapSCSS, gulp.series("bootstrapSCSS"));
